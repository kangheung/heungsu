...
function buildCalendar(){
  ...
  for(i=1; i <= lastDate.getDate(); i++){
    ...
    var week = document.querySelectorAll('#calendar tr td ');
    var newDiv = document.createElement("div"); 
    var newBtn = document.createElement('button');
    
  
    newBtn.append(i);
    week[i].appendChild(newBtn);  // 이번달 날짜만큼 버튼 생성
    ...
    if(cnt % 7 == 0){
      ...
      if(cnt % 7 == 6) {
        ...
        newDiv.append("85,000");
        week[i].appendChild(newBtn).append(newDiv);
      }
      else if(cnt % 7 == 5){
        newDiv.append("74,000");
        week[i].appendChild(newBtn).append(newDiv);
      } else {
        newDiv.append("65,000");
        week[i].appendChild(newBtn).append(newDiv);
      }
